...
router.put('/password', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Validate new password - must match frontend requirements
    if (newPassword.length < 8) {
      res.status(400).json({ success: false, error: 'Yangi parol kamida 8 ta belgi bo\'lishi kerak' });
      return;
    }
    if (!/[A-Z]/.test(newPassword)) {
      res.status(400).json({ success: false, error: 'Parol kamida bitta katta harf bo\'lishi kerak' });
      return;
    }
    if (!/[a-z]/.test(newPassword)) {
      res.status(400).json({ success: false, error: 'Parol kamida bitta kichik harf bo\'lishi kerak' });
      return;
    }
    if (!/[0-9]/.test(newPassword)) {
      res.status(400).json({ success: false, error: 'Parol kamida bitta raqam bo\'lishi kerak' });
      return;
    }
