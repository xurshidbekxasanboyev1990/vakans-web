...
import { notifyNewApplication, notifyApplicationStatus } from '../socket/index';
...
router.post('/', authenticate, requireRole('worker'), async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Get job info
    const jobInfo = await query('SELECT title FROM jobs WHERE id = $1', [jobId]);
    ...
    notifyNewApplication(job.employer_id, {
      id: result.rows[0].id,
      jobId,
      jobTitle,
      workerName,
...
router.put('/:id', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const jobResult = await query('SELECT title FROM jobs WHERE id = $1', [app.job_id]);
    ...
    notifyApplicationStatus(app.worker_id, {
      id,
      jobId: app.job_id,
      jobTitle,
