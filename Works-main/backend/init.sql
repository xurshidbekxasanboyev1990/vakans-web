...
CREATE TABLE IF NOT EXISTS users (
    ...
    -- SECURITY: plain_password column removed - never store plain passwords!
...
-- Add likes/dislikes count to jobs table
ALTER TABLE jobs ADD COLUMN IF NOT EXISTS likes_count INTEGER DEFAULT 0;
ALTER TABLE jobs ADD COLUMN IF NOT EXISTS dislikes_count INTEGER DEFAULT 0;
...
-- Update chat room last_message_at trigger
CREATE OR REPLACE FUNCTION update_chat_last_message()
RETURNS TRIGGER AS $$
BEGIN
    UPDATE chat_rooms SET last_message_at = NOW() WHERE id = NEW.chat_id;
    RETURN NEW;
END;
$$ language 'plpgsql';
...
-- Bu faqat DEVELOPMENT/TEST uchun default admin hisobi
-- PRODUCTION uchun:
-- 1. Bu INSERT ni olib tashlang yoki yangi hash yarating
-- 2. Admin birinchi login'dan keyin parolni o'zgartirishi SHART
-- 3. Hech qachon parolni sharhlarda yozmang
-- ============================================
INSERT INTO users (phone, email, password_hash, first_name, last_name, user_type, region, is_verified)
VALUES (
    '+998996983806',
    'admin@vakans.uz',
    -- Hash yaratish: node -e "require('bcryptjs').hash('YOUR_SECURE_PASSWORD', 12).then(console.log)"
    '$2b$12$rNRjTGw6EfVHRVbFjJKdOe5.p1kG4vIFz.4g5PXsRhGvKq/RyWBDq',
    'Admin',
    'XOJISAID',
    'admin',
    'Toshkent',
...
-- ============================================
-- DEMO ACCOUNTS (FAQAT DEVELOPMENT UCHUN!)
-- ============================================
-- ⚠️ PRODUCTION UCHUN: Bu qismni butunlay o'chirib tashlang!
-- Yangi hash yaratish:
-- node -e "require('bcryptjs').hash('YourPassword123!', 12).then(console.log)"
-- ============================================

-- Demo Employer (Ish beruvchi) - FAQAT TEST UCHUN
INSERT INTO users (phone, email, password_hash, first_name, last_name, user_type, region, company_name, is_verified)
VALUES (
    '+998901234567',
    'employer@demo.uz',
    '$2b$12$LxWf.wKHPv8YY8EWGqP4e.qR2nC5kK1hS6eHxGpVwXnq0FqVQBP8C',
    'Sardor',
    'Karimov',
    'employer',
    'Toshkent',
...
-- Demo Worker (Ish izlovchi) - FAQAT TEST UCHUN
INSERT INTO users (phone, email, password_hash, first_name, last_name, user_type, region, skills, is_verified)
VALUES (
    '+998907654321',
    'worker@demo.uz',
    '$2b$12$mNvP3qO8kL2dF4gH5jR6tuhY7wB9cZ1aS0xM3nK4pQ8rT5vU6wX2i',
    'Aziz',
    'Toshmatov',
    'worker',
    'Samarqand',
    ARRAY['Qurilish', 'Ta''mirlash', 'Tozalash'],
    true
) ON CONFLICT (phone) DO NOTHING;
