...
import { io, notifyNewJob, notifyJobUpdate } from '../socket/index';
...
router.post('/', authenticate, requireRole('employer', 'admin'), async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Real-time: Notify all connected clients about new job
    io.emit('job:new', {
      id: job.id,
      title: job.title,
...
router.put('/:id', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Real-time: Notify about job update
    io.emit('job:update', {
      id: updatedJob.id,
...
router.delete('/:id', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Real-time: Notify about job deletion
...
// ============================================
// POST /jobs/:id/view - Ish ko'rildi
// ============================================
...
// ============================================
// POST /jobs/:id/reaction - Like/Dislike
// ============================================
router.post('/:id/reaction', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!['like', 'dislike'].includes(type)) {
      res.status(400).json({ success: false, error: 'Noto\'g\'ri reaksiya turi' });
      return;
    }

    // Check if user already reacted
    const existingReaction = await query(
      `SELECT * FROM job_reactions WHERE job_id = $1 AND user_id = $2`,
      [id, req.user!.id]
    );

    if (existingReaction.rows.length > 0) {
      const currentType = existingReaction.rows[0].reaction_type;
      
      if (currentType === type) {
        // Remove reaction
        await query(
          `DELETE FROM job_reactions WHERE job_id = $1 AND user_id = $2`,
          [id, req.user!.id]
        );
        
        // Update job counter
        if (type === 'like') {
          await query(`UPDATE jobs SET likes_count = GREATEST(0, likes_count - 1) WHERE id = $1`, [id]);
        } else {
          await query(`UPDATE jobs SET dislikes_count = GREATEST(0, dislikes_count - 1) WHERE id = $1`, [id]);
        }
        
        res.json({ success: true, action: 'removed', type: null });
        return;
      } else {
        // Change reaction
        await query(
          `UPDATE job_reactions SET reaction_type = $1 WHERE job_id = $2 AND user_id = $3`,
          [type, id, req.user!.id]
        );
        
        // Update counters
        if (type === 'like') {
          await query(`UPDATE jobs SET likes_count = likes_count + 1, dislikes_count = GREATEST(0, dislikes_count - 1) WHERE id = $1`, [id]);
        } else {
          await query(`UPDATE jobs SET dislikes_count = dislikes_count + 1, likes_count = GREATEST(0, likes_count - 1) WHERE id = $1`, [id]);
        }
        
        res.json({ success: true, action: 'changed', type });
        return;
      }
    }
    ...
    // Add new reaction
    await query(
      `INSERT INTO job_reactions (job_id, user_id, reaction_type) VALUES ($1, $2, $3)`,
      [id, req.user!.id, type]
    );

    // Update job counter
    if (type === 'like') {
      await query(`UPDATE jobs SET likes_count = likes_count + 1 WHERE id = $1`, [id]);
    } else {
      await query(`UPDATE jobs SET dislikes_count = dislikes_count + 1 WHERE id = $1`, [id]);
    }
...
// ============================================
// GET /jobs/:id/reaction - Foydalanuvchi reaksiyasini olish
// ============================================
router.get('/:id/reaction', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await query(
      `SELECT reaction_type FROM job_reactions WHERE job_id = $1 AND user_id = $2`,
      [id, req.user!.id]
    );
    ...
    res.json({ 
      success: true, 
      data: { 
        userReaction: result.rows[0]?.reaction_type || null 
      } 
    });
  } catch (error) {
...
// ============================================
// POST /jobs/:id/save - Ishni saqlash/olib tashlash
// ============================================
...
// ============================================
// GET /jobs/saved - Saqlangan ishlar
// ============================================
