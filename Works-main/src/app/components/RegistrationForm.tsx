...
export function RegistrationForm({ userType, onRegister, onSwitchToLogin, isLoading }: RegistrationFormProps) {
  ...
  return (
    <Card className="w-full max-w-[95vw] sm:max-w-md mx-auto bg-card border-2 border-border shadow-xl rounded-xl sm:rounded-2xl">
      ...
      <CardContent className="px-4 sm:px-6 pb-4 sm:pb-6">
        <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
          ...
          <div className="space-y-1.5 sm:space-y-2">
            <Label htmlFor="phone" className="flex items-center gap-1 sm:gap-1.5 text-xs sm:text-sm">
            ...
            <p className="text-[10px] sm:text-xs text-muted-foreground">{t('phoneAsLogin')}</p>
          </div>

          <div className="space-y-1.5 sm:space-y-2">
            <Label htmlFor="region" className="text-xs sm:text-sm">{t('region')}</Label>
            <Select value={region} onValueChange={(val) => {
              setRegion(val);
              if (errors.region) setErrors({...errors, region: ''});
            }} required>
              <SelectTrigger id="region" className="h-9 sm:h-11 text-sm sm:text-base">
                <SelectValue placeholder={t('selectRegion')} />
              </SelectTrigger>
              <SelectContent>
                {REGIONS.map((regionName) => (
                  <SelectItem key={regionName} value={regionName} className="text-xs sm:text-sm">
                    {regionName}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
            {errors.region && <p className="text-xs sm:text-sm text-red-500">{errors.region}</p>}
          </div>

          <div className="space-y-1.5 sm:space-y-2">
            <Label htmlFor="password" className="flex items-center gap-1 sm:gap-1.5 text-xs sm:text-sm">
            ...
            <div className="relative">
              ...
              <button
                ...
                className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground p-1 z-10"
            ...
            {password && (
              <p className={`text-[10px] sm:text-xs ${getPasswordStrength(password).color}`}>
          ...
          <Button type="submit" className="w-full h-9 sm:h-11 mt-4 sm:mt-6 text-sm sm:text-base" disabled={isLoading}>
