...
function LandingPage() {
  ...
  const [realStats, setRealStats] = useState({ jobs: 0, workers: 0, employers: 0 });

  // Real statistika olish
  useEffect(() => {
    const fetchRealStats = () => {
      try {
        // localStorage dan real ma'lumotlarni olish
        const jobs = JSON.parse(localStorage.getItem('employer_jobs') || '[]');
        const users = JSON.parse(localStorage.getItem('demo_users') || '[]');
        
        const workers = users.filter((u: { userType: string }) => u.userType === 'worker').length;
        const employers = users.filter((u: { userType: string }) => u.userType === 'employer').length;
        ...
        setRealStats({ jobs: 0, workers: 0, employers: 0 });
      }
    };
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      {/* Main Content */}
      <div className="relative flex items-center justify-center min-h-[100dvh] p-3 sm:p-4 overflow-y-auto">
        <div className="w-full max-w-5xl space-y-6 sm:space-y-8 lg:space-y-12 py-4 sm:py-0">
          {/* Hero Section */}
          <div className="text-center space-y-3 sm:space-y-4 lg:space-y-6 animate-fade-in-down">
            {/* Logo Badge - Real statistika */}
            <div className="inline-flex items-center gap-1.5 sm:gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border border-gray-200 dark:border-gray-700 shadow-lg animate-bounce-soft">
              <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-green-500 rounded-full animate-pulse" />
              <span className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-300">
                {realStats.jobs > 0 ? `${realStats.jobs} ta vakansiya mavjud` : 'Yangi vakansiyalar kutilmoqda'}
              </span>
            </div>

            {/* Main Title */}
            <h1 
              className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold cursor-pointer select-none"
            ...
            {/* Subtitle */}
            <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-600 dark:text-gray-300 max-w-2xl mx-auto leading-relaxed px-2">
            ...
            <div className="flex flex-wrap justify-center gap-1.5 sm:gap-2 max-w-xl mx-auto px-2">
              {['Toshkent', 'Samarqand', 'Buxoro', 'Andijon', "Farg'ona"].map((region, i) => (
                <span 
                  key={region}
                  className="px-2 py-0.5 sm:px-3 sm:py-1 text-[10px] sm:text-xs font-medium bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm rounded-full border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 animate-fade-in-up"
                  style={{ animationDelay: `${i * 0.1}s` }}
                >
                  {region}
                </span>
              ))}
              <span className="px-2 py-0.5 sm:px-3 sm:py-1 text-[10px] sm:text-xs font-medium bg-blue-100 dark:bg-blue-900/50 rounded-full text-blue-600 dark:text-blue-400">
                +9 viloyat
              </span>
            </div>
          </div>
          ...
          {/* Cards Section */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 lg:gap-8 px-1">
            {/* Worker Card */}
            <div 
              className="group relative animate-fade-in-up stagger-1"
              onClick={() => navigate('/login/worker')}
            >
              <div className="absolute inset-0 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-xl sm:rounded-2xl blur-xl opacity-0 group-hover:opacity-30 transition-opacity duration-500" />
              <Card className="relative border-0 bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl shadow-xl hover:shadow-2xl transition-all duration-500 cursor-pointer overflow-hidden rounded-xl sm:rounded-2xl">
                {/* Decorative gradient line */}
                <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-blue-500 to-cyan-500" />
                
                <CardHeader className="pb-3 sm:pb-4 pt-5 sm:pt-8 px-4 sm:px-6">
                  <div className="flex items-center gap-3 sm:gap-4">
                    <div className="w-12 h-12 sm:w-14 md:w-16 sm:h-14 md:h-16 rounded-xl sm:rounded-2xl bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 flex-shrink-0">
                      <Users className="w-6 h-6 sm:w-7 md:w-8 sm:h-7 md:h-8 text-white" />
                    </div>
                    <div className="min-w-0">
                      <CardTitle className="text-lg sm:text-xl md:text-2xl font-bold">{t('findWorkTitle')}</CardTitle>
                ...
                <CardContent className="space-y-2 sm:space-y-3 pb-5 sm:pb-8 px-4 sm:px-6">
                  {/* Features */}
                  <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-3 sm:mb-4">
                    {['Bepul', 'Tez', 'Oson'].map((feature) => (
                      <span key={feature} className="px-1.5 sm:px-2 py-0.5 sm:py-1 text-[10px] sm:text-xs bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 rounded-md">
                        ✓ {feature}
                      </span>
                    ))}
                  </div>
                  ...
                  <Button 
                    className="w-full h-10 sm:h-12 text-sm sm:text-base font-semibold bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white border-0 shadow-lg hover:shadow-xl transition-all duration-300 group-hover:scale-[1.02]"
                    onClick={(e) => { e.stopPropagation(); navigate('/login/worker'); }}
                  >
                    {t('login')} <ArrowRight className="ml-1.5 sm:ml-2 w-4 h-4 sm:w-5 sm:h-5 group-hover:translate-x-1 transition-transform" />
                  </Button>
                  <Button 
                    variant="ghost" 
                    className="w-full h-8 sm:h-10 text-xs sm:text-sm text-gray-600 dark:text-gray-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30"
            ...
            >
              <div className="absolute inset-0 bg-gradient-to-r from-purple-500 to-pink-500 rounded-xl sm:rounded-2xl blur-xl opacity-0 group-hover:opacity-30 transition-opacity duration-500" />
              <Card className="relative border-0 bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl shadow-xl hover:shadow-2xl transition-all duration-500 cursor-pointer overflow-hidden rounded-xl sm:rounded-2xl">
                {/* Decorative gradient line */}
                ...
                <CardHeader className="pb-3 sm:pb-4 pt-5 sm:pt-8 px-4 sm:px-6">
                  <div className="flex items-center gap-3 sm:gap-4">
                    <div className="w-12 h-12 sm:w-14 md:w-16 sm:h-14 md:h-16 rounded-xl sm:rounded-2xl bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:-rotate-3 transition-all duration-300 flex-shrink-0">
                      <Briefcase className="w-6 h-6 sm:w-7 md:w-8 sm:h-7 md:h-8 text-white" />
                    </div>
                    <div className="min-w-0">
                      <CardTitle className="text-lg sm:text-xl md:text-2xl font-bold">{t('postJobTitle')}</CardTitle>
                      <CardDescription className="text-sm sm:text-base mt-0.5 sm:mt-1">
                        {t('postJobDesc')}
                      </CardDescription>
                    </div>
                ...
                <CardContent className="space-y-2 sm:space-y-3 pb-5 sm:pb-8 px-4 sm:px-6">
                  ...
                  <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-3 sm:mb-4">
                    {['Tezkor', 'Samarali', 'Arzon'].map((feature) => (
                      <span key={feature} className="px-1.5 sm:px-2 py-0.5 sm:py-1 text-[10px] sm:text-xs bg-purple-100 dark:bg-purple-900/50 text-purple-600 dark:text-purple-400 rounded-md">
                        ✓ {feature}
                      </span>
                    ))}
                  </div>
                  ...
                  <Button 
                    className="w-full h-10 sm:h-12 text-sm sm:text-base font-semibold bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white border-0 shadow-lg hover:shadow-xl transition-all duration-300 group-hover:scale-[1.02]"
                  ...
                  <Button 
                    variant="ghost" 
                    className="w-full h-8 sm:h-10 text-xs sm:text-sm text-gray-600 dark:text-gray-400 hover:text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-900/30"
          ...
          {/* Stats Section - Real statistika */}
          <div className="grid grid-cols-3 gap-2 sm:gap-4 max-w-2xl mx-auto animate-fade-in-up stagger-3 px-1">
            {[
              { value: String(realStats.jobs), label: 'Vakansiyalar' },
              { value: String(realStats.workers), label: 'Ishchilar' },
              { value: String(realStats.employers), label: 'Ish beruvchilar' },
            ].map((stat, i) => (
              <div 
                key={stat.label}
                className="text-center p-2 sm:p-3 md:p-4 rounded-lg sm:rounded-xl bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm border border-gray-200 dark:border-gray-700"
              >
                <div className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-gradient">{stat.value}</div>
                <div className="text-[10px] sm:text-xs md:text-sm text-gray-500 dark:text-gray-400">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
...
function RoleSelectionPage() {
  ...
  return (
    <div className="min-h-[100dvh] bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 overflow-y-auto">
      <div className="fixed top-2 right-2 sm:top-4 sm:right-4 z-50">
        <ThemeToggle />
      </div>

      <div className="flex items-center justify-center min-h-[100dvh] p-3 sm:p-4">
        <div className="w-full max-w-[95vw] sm:max-w-2xl space-y-4 sm:space-y-6 lg:space-y-8 py-4 sm:py-0">
          <div className="text-center space-y-1.5 sm:space-y-2">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold">{t('selectRole')}</h1>
            <p className="text-sm sm:text-base text-muted-foreground px-2">{t('platformDescription')}</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 md:gap-6">
            <Card className="border-2 hover:shadow-xl transition-all rounded-xl sm:rounded-2xl">
              ...
              <CardContent className="space-y-2 sm:space-y-3 px-4 sm:px-6 pb-4 sm:pb-6">
                <Button className="w-full h-9 sm:h-10 text-sm sm:text-base" onClick={() => navigate('/login/worker')}>
                  {t('login')}
                </Button>
                <Button variant="outline" className="w-full h-8 sm:h-10 text-xs sm:text-sm" onClick={() => navigate('/register/worker')}>
                  {t('register')}
                </Button>
              </CardContent>
            </Card>
            ...
            <Card className="border-2 hover:shadow-xl transition-all rounded-xl sm:rounded-2xl">
              ...
              <CardContent className="space-y-2 sm:space-y-3 px-4 sm:px-6 pb-4 sm:pb-6">
                <Button className="w-full h-9 sm:h-10 text-sm sm:text-base" onClick={() => navigate('/login/employer')}>
                  {t('login')}
                </Button>
                <Button variant="outline" className="w-full h-8 sm:h-10 text-xs sm:text-sm" onClick={() => navigate('/register/employer')}>
                  {t('register')}
                </Button>
              </CardContent>
            </Card>
...
function LoginPage({ userType }: { userType: 'worker' | 'employer' }) {
  ...
  return (
    <div className="min-h-[100dvh] bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
      <div className="fixed top-2 right-2 sm:top-4 sm:right-4 z-50">
        <ThemeToggle />
      </div>
      
      <div className="w-full max-w-[95vw] sm:max-w-md space-y-3 sm:space-y-4 py-4 sm:py-0">
...
function AdminDashboardPage() {
  ...
  const handleResetPassword = (userId: string) => {
    const users = getDemoUsersFromStorage();
    const targetUser = users.find((u: DemoUserType) => u.id === userId);
    if (targetUser) {
      // 🔐 Xavfsiz random parol generatsiya qilish
      ...
      const updatedUsers = users.map((u: DemoUserType) => 
        u.id === userId ? { ...u, password: btoa(newPassword) } : u
      );
      localStorage.setItem('demo_users', JSON.stringify(updatedUsers));
  ...
  const handleSendMessage = (userId: string, message: string) => {
    ...
    if (targetUser) {
      ...
      // TODO: Real SMS API orqali yuborish
    }
  };
...
function AppRoutes() {
  return (
    <Routes>
      ...
      {/* /select-role olib tashlandi - LandingPage dan to'g'ridan-to'g'ri login'ga o'tiladi */}
