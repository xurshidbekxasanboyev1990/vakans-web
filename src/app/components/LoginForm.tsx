...
export function LoginForm({ userType, onLogin, onSwitchToRegister, isLoading }: LoginFormProps) {
  ...
  return (
    <div className="w-full max-w-md mx-auto animate-fade-in-up">
      ...
      <Card className="relative border-0 bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl shadow-2xl rounded-2xl overflow-hidden">
        {/* Top gradient bar */}
        <div className={`absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r ${gradientClass}`} />
        
        {/* Language & Info Buttons - Top Right */}
        <div className="absolute top-4 right-4 flex items-center gap-2 z-10">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">
            ...
            <DropdownMenuContent align="end" className="w-40">
              {LANGUAGES.map((lang) => (
                <DropdownMenuItem
                  ...
                  className={`flex items-center gap-2 cursor-pointer ${language === lang.code ? 'bg-blue-50 dark:bg-blue-900/30' : ''}`}
                >
        ...
        <CardHeader className="space-y-4 pb-6 pt-8">
          ...
          <div className="flex justify-center">
            <div className={`w-20 h-20 rounded-2xl bg-gradient-to-br ${gradientClass} flex items-center justify-center shadow-xl animate-bounce-soft`}>
              {isWorker 
          ...
          <div className="text-center space-y-2">
            ...
            <CardDescription className="text-base flex items-center justify-center gap-2">
              <span className={`px-3 py-1 rounded-full text-sm font-medium bg-${accentColor}-100 dark:bg-${accentColor}-900/50 text-${accentColor}-600 dark:text-${accentColor}-400`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              <Label htmlFor="phone" className="flex items-center gap-2 text-sm font-medium">
                <Phone className={`w-4 h-4 text-${accentColor}-500`} />
                {t('phone')}
              </Label>
              <div className="relative">
                <Input
                  ...
                  required
                  className={`h-12 pl-4 pr-4 text-base border-2 rounded-xl transition-all duration-300 focus:border-${accentColor}-500 focus:ring-2 focus:ring-${accentColor}-500/20`}
                />
              </div>
              {errors.phone && (
            ...
            <div className="space-y-2">
              <Label htmlFor="password" className="flex items-center gap-2 text-sm font-medium">
                <Shield className={`w-4 h-4 text-${accentColor}-500`} />
                {t('password')}
              </Label>
              <div className={`flex items-center w-full h-12 border-2 rounded-xl overflow-hidden transition-all duration-300 focus-within:border-${accentColor}-500 focus-within:ring-2 focus-within:ring-${accentColor}-500/20 bg-background`}>
                <input
                  id="password"
                  ...
                  placeholder="••••••••"
                  required
                  className="flex-1 h-full px-4 text-base bg-transparent outline-none border-none text-foreground placeholder:text-muted-foreground"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className={`flex items-center justify-center w-10 h-9 mx-1.5 rounded-lg bg-${accentColor}-500 text-white hover:bg-${accentColor}-600 transition-colors`}
                >
            ...
            <Button 
              type="submit" 
              className={`w-full h-12 text-base font-semibold bg-gradient-to-r ${gradientClass} hover:opacity-90 text-white border-0 shadow-lg hover:shadow-xl transition-all duration-300 rounded-xl mt-2`}
              disabled={isLoading}
            >
              {isLoading ? (
      ...
      {showAbout && (
        <>
          ...
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-sm bg-white dark:bg-gray-900 rounded-2xl shadow-2xl z-50 p-6">
            <div className="text-center mb-4">
              ...
              <h3 className="text-xl font-bold text-gray-900 dark:text-white">Vakans.uz</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t('aboutDescription')}</p>
            </div>
              ...
              return (
                <div className="grid grid-cols-2 gap-2 mb-4">
                  <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-center">
                    <p className="text-xl font-bold text-blue-600">{stats.workers}</p>
                    <p className="text-xs text-gray-500">{t('worker')}</p>
                  </div>
                  <div className="p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg text-center">
                    <p className="text-xl font-bold text-purple-600">{stats.employers}</p>
                    <p className="text-xs text-gray-500">{t('employer')}</p>
                  </div>
                  ...
                  <div className="p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg text-center">
                    <p className="text-xl font-bold text-amber-600">{stats.completedJobs}</p>
            ...
            <Button onClick={() => setShowAbout(false)} className="w-full">{t('close')}</Button>
          </div>
        </>
      )}
      ...
      {showSupport && (
        <>
          ...
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-sm bg-white dark:bg-gray-900 rounded-2xl shadow-2xl z-50 p-6 max-h-[90vh] overflow-y-auto">
            <div className="text-center mb-4">
              <div className="w-16 h-16 mx-auto rounded-2xl bg-gradient-to-br from-pink-500 to-rose-600 flex items-center justify-center mb-3">
                <MessageCircle className="w-8 h-8 text-white" />
              </div>
              <h3 className="text-xl font-bold text-gray-900 dark:text-white">{t('support')}</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t('contactUs')}</p>
            </div>
                ...
                <div className="space-y-3 mb-4">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{supportInfo.telegram}</p>
                    </div>
                  </a>
                  ...
                  >
                    ...
                    <div>
                      <p className="font-medium text-gray-900 dark:text-white text-sm">{t('phone')}</p>
            ...
            <Button onClick={() => setShowSupport(false)} className="w-full">{t('close')}</Button>
          </div>
        </>
      )}
    </div>
...
// Parol tiklash so'rovi komponenti
  ...
  if (isSubmitted) {
    return (
      <div className="p-4 bg-green-50 dark:bg-green-900/20 rounded-lg text-center">
        ...
        <h4 className="font-semibold text-green-700 dark:text-green-400">So'rov yuborildi!</h4>
      ...
      <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-semibold text-blue-700 dark:text-blue-400">Javobni tekshirish</h4>
          </div>
          <button 
        ...
        <div className="space-y-3">
          <div>
            <Label className="text-xs text-gray-600">Telefon raqamingiz</Label>
          ...
          {statusError && (
            <div className="p-3 bg-red-100 dark:bg-red-900/20 rounded-lg text-red-700 dark:text-red-400 text-sm">
              {statusError}
            </div>
          )}
              ...
              {recoveryStatus.status === 'resolved' ? (
                <div className="space-y-3">
                  <div className="flex items-center gap-2 text-green-700 dark:text-green-400">
                    ...
                    <span className="font-semibold">Admin javob berdi!</span>
                  </div>
                  ...
                  {recoveryStatus.newPassword && (
                    <div className="p-3 bg-white dark:bg-gray-800 rounded-lg border-2 border-green-500">
                      <span className="text-xs text-gray-500 block mb-1">Yangi parolingiz:</span>
                ...
                <div className="text-center">
                  <div className="w-10 h-10 mx-auto mb-2 border-4 border-yellow-400 border-t-transparent rounded-full animate-spin" />
                  <p className="font-medium text-yellow-700 dark:text-yellow-400">So'rov ko'rib chiqilmoqda</p>
    ...
    <div className="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <h4 className="font-semibold text-purple-700 dark:text-purple-400">Parolni tiklash</h4>
        </div>
        <button 
      ...
      <div className="space-y-3">
        ...
        <div>
          <Label className="text-xs text-gray-600">Qo'shimcha xabar (ixtiyoriy)</Label>
          <Input
            value={message}
